...
def _top_shap_features(
    ...
    candidate_ids = np.asarray(
        [idx for idx in candidate_ids if _is_content_feature(str(feature_names[idx]))],
        dtype=int,
    )
...
def _is_content_feature(feature: str) -> bool:
    """Keep SHAP terms that contain at least one non-stopword content token."""
    tokens = [token for token in feature.lower().split() if token]
    if not tokens:
        return False
    return any(token not in STOPWORDS and len(token) >= 3 for token in tokens)
...
def logistic_regression_shap_artifacts(
    ...
    for class_index, class_name in enumerate(class_names):
        ...
        ranked_ids = mean_abs.argsort()[::-1]
        top_ids = [feature_id for feature_id in ranked_ids if _is_content_feature(feature_names[feature_id])][:top_n]
        for rank, feature_id in enumerate(top_ids, start=1):
            global_rows.append(
                {
                    "class": class_name,
